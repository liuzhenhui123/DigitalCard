...
// 音频转码worker
let recorderWorker = new Worker('StreamingAssets/VoiceJs/iat_Js/transformpcm.worker.js')
// 记录处理的缓存音频
let buffer = []
...
class IatRecorder {
  ...
  connectWebsocket () {
    ...
    var apiKey = iat_APIKEY
    var apiSecret = iat_APISECRET
  ...
  wsOpened () {
    ...
    var params = {
      ...
      'business': {
        'language': iat_language, //小语种可在控制台--语音听写（流式）--方言/语种处添加试用
        'domain': 'iat',
...
class IatTaste {
  ...
  setResult (data) {
    ...
    UnityIns.SendMessage('RainierVoiceSystem','RecognitionCallBack',str);
	console.log(str);
